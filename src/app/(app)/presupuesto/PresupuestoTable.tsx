...
type Plan = { id: string; items: { categoryId: string; month: number; plannedAmount: number }[] };
...
function CategoriaRow({
  ...
  function guardarMontos() {
    startTransition(async () => {
      await actualizarPresupuestoCategoria(planId, year, categoria.id, montos);
      setSaved(true);
  ...
  return (
    <tr className="border-b border-slate-100 last:border-0">
      ...
      <td className="px-2 py-2">
        {readOnly ? (
          <span className="text-xs text-slate-600">{TIPO_LABEL[type]}</span>
        ) : (
          <select
            value={type}
      ...
      <td className="px-2 py-2">
        {readOnly ? (
          <span className="text-xs text-slate-600">{PRIORIDAD_LABEL[priority]}</span>
        ) : (
          <select
            value={priority}
      ...
      {montos.map((m, idx) => (
        <td key={idx} className="px-1 py-2">
          {readOnly ? (
            <span className={clsx("block text-right text-xs font-medium", esIngreso ? "text-[#2a78d6]" : "text-[#d03b3b]")}>
              <Money value={m} />
            </span>
          ) : (
            <input
...
export function PresupuestoTable({
  ...
  const montosPorCategoria = useMemo(() => {
    ...
    for (const i of plan.items) {
      const arr = map.get(i.categoryId);
      if (arr) arr[i.month - 1] = i.plannedAmount;
    }
    return map;
  }, [categorias, plan.items]);
